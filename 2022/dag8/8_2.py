...
def main():
    with open(sys.argv[1], 'r') as f:
        ...
        nested_elems_to_update = [[(i, j) for j, elem in enumerate(row) if elem == -1] for i, row in enumerate(is_visible)]
        elems_to_update = functools.reduce(lambda a, b: a + b, nested_elems_to_update)
        ...
        elems_to_update = functools.reduce(
            lambda a, b: a + b, nested_elems_to_update)
        max_score = 0
        for elem in elems_to_update:
            if is_visible[elem[0]][elem[1]]:
                i = 1
                while elem[0]-i > 0 and data[elem[0]][elem[1]] > data[elem[0]-i][elem[1]]:
                    i += 1
                right_length = i
                i = 1
                while elem[0]+i < len(data)-1 and data[elem[0]][elem[1]] > data[elem[0]+i][elem[1]]:
                    i += 1
                left_length = i
                i = 1
                while elem[1]-i > 0 and data[elem[0]][elem[1]] > data[elem[0]][elem[1]-i]:
                    i += 1
                up_length = i
                i = 1
                while elem[1]+i < len(data[0])-1 and data[elem[0]][elem[1]] > data[elem[0]][elem[1]+i]:
                    #print('d', elem[1]+i)
                    i += 1
                down_length = i
                temp_score = right_length * left_length * up_length * down_length
                #print(up_length)
                #print(left_length)
                #print(down_length)
                #print(right_length)
