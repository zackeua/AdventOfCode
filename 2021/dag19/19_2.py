...
# for faster execution of my input
queue = [17, 20, 24, 29, 10, 23, 11, 30, 7, 19, 21, 25, 27, 33, 4, 5, 6, 8, 12, 14, 22, 31, 32, 1, 2, 3, 9, 13, 15, 16, 18, 26, 28]
while queue != []:
    index = queue[0]
    print(queue)
    queue = queue[1:]
...
for pos_a in beacon_pos:
    for pos_b in beacon_pos:
        temp = sum([abs(a-b) for a, b in zip(pos_a, pos_b)])
