    ...
    for key in antennas.keys():
        for a1 in antennas[key]:
            for a2 in antennas[key]:
                ...
                if p1 == a2:
                    while x_min < p2[0] < x_max and y_min < p2[1] < y_max:
                        antinode_locations.add(p2)
                        p2 = (p2[0] + diff_vector[0], p2[1] + diff_vector[1])
                    while x_min < p3[0] < x_max and y_min < p3[1] < y_max:
                        antinode_locations.add(p3)
                        p3 = (p3[0] - diff_vector[0], p3[1] - diff_vector[1])
                elif p2 == a2:
                    while x_min < p1[0] < x_max and y_min < p1[1] < y_max:
                        antinode_locations.add(p1)
                        p1 = (p1[0] - diff_vector[0], p1[1] - diff_vector[1])
                    while x_min < p4[0] < x_max and y_min < p4[1] < y_max:
                        antinode_locations.add(p4)
                        p4 = (p4[0] + diff_vector[0], p4[1] + diff_vector[1])
                else:
                    assert False
