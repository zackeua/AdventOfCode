...
class Hand:
    ...
    def five_of_a_kind(self) -> bool:
        return self.cards.count(self.cards[0]) == (5- self.cards.count('J')) 
    def four_of_a_kind(self) -> bool:
        j_s = self.cards.count('J')
        return self.cards.count(self.cards[0]) == (4 - j_s)  or self.cards.count(self.cards[1]) == (4 - j_s)
    ...
    def n_of_a_kind(self, n) -> bool:
        ...
        j_s = self.cards.count('J')
        for i in range(5):
            if self.cards.count(self.cards[i]) == (n - j_s):
                n_kind = True
        return n_kind
